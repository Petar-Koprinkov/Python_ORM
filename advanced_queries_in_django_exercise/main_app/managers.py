...
class RealEstateListingManager(models.Manager):
    ...
    def popular_locations(self):
        return self.values('location').annotate(location_count=Count('location')).order_by('-location_count', 'location')[0:2]
...
class VideoGameManager(models.Manager):
    ...
    def highest_rated_game(self):
        return self.annotate(max_rating=Max('rating')).order_by('-rating').first()

    def lowest_rated_game(self):
        return self.annotate(min_rating=Min('rating')).order_by('rating').first()

    def average_rating(self):
        result = self.aggregate(total=Avg('rating'))['total']
        return f"{result:.1f}"
