...
from main_app.models import ArtworkGallery, Laptop, BrandChoices, OperationSystemChoices, ChessPlayer, Meal, Dungeon, Workout
...
def update_operation_systems():
    Laptop.objects.filter(brand=BrandChoices.ASUS).update(operation_system=OperationSystemChoices.WINDOWS)
    Laptop.objects.filter(brand=BrandChoices.APPLE).update(operation_system=OperationSystemChoices.MACOS)
    Laptop.objects.filter(brand__in=(BrandChoices.DELL, BrandChoices.ACER)).update(operation_system=OperationSystemChoices.LINUX)
    Laptop.objects.filter(brand=BrandChoices.LENOVO).update(operation_system=OperationSystemChoices.CHROME_OS)
...
def delete_chess_players():
    ChessPlayer.objects.filter(title=ChessPlayer._meta.get_field('title').get_default()).delete()
...
def change_chess_games_lost():
    ChessPlayer.objects.filter(title=ChessPlayer._meta.get_field('title').get_default()).update(games_lost=25)
...
def update_low_calorie_meals():
    Meal.objects.filter(meal_type__in=('Dinner', 'Breakfast')).update(calories=400)
...
def show_hard_dungeons():
    ...
    return '\n'.join([f"{dungeons.name} is guarded by {dungeons.boss_name} who has {dungeons.boss_health} "
                      f"health points!" for dungeons in hard_dungeons])
